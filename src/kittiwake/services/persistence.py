...
class SavedAnalysisRepository:
    ...
    def _init_database(self):
        ...
        if table_exists:
            # Check if id column has IDENTITY (auto-increment)
            ...
            try:
                ...
                # If column_default doesn't contain 'nextval' or similar, we need to recreate
        ...
        # Create saved_analyses table with proper auto-increment
        if not table_exists:
            conn.execute("""
            ...
            if needs_restore and restore_data:
                for row in restore_data:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, list(row[1:]))  # Skip the old id, let it auto-generate
